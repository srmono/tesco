...
./cbq --script="select \* from \`academic-data\`"
## This will be an error 

./cbq -u admin -p bvsrao -engine=http://127.0.0.1:8091/ 
...
select student_name, python_score, wallet_balance
FROM `academic-data`;

select student_name, python_score, sports_medals.gold, dorm_fee[2]
FROM `academic-data`;

insert into `academic-data` (key, value)
values ("stu_02", { "student_id": 102,
                    "student_name": "David",
                    "java_score": 40,
                    "python_score": 60,
                    "js_score": 60,
...
./cbq -u admin -p bvsrao \
-engine=http://127.0.0.1:8091/ \
--script="select student_name, python_score, wallet_balance from \`academic-data\`"

subl ~/student_query.nosql

./cbq -u admin -p bvsrao \
-engine=http://127.0.0.1:8091/ \
--file ~/student_query.nosql
...
-d 'statement=select student_name, python_score, wallet_balance from `academic-data`' \
-u admin:bvsrao

curl -v http://localhost:8093/query/service \
-d 'statement=select student_name, wallet_balance 
from `academic-data`
where wallet_balance > 800' \
-u admin:bvsrao
